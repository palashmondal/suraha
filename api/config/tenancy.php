...
return [
     ...
     *
     * SINGLE-DATABASE TENANCY (per SURAHA_BUILD_PROMPT §4/§12.1): we keep ONE central
     * PostgreSQL and scope tenant-owned rows by `tenant_id` via the BelongsToTenant global
     * scope — we do NOT switch databases per tenant. So the DB/Cache/Filesystem/Queue
     * bootstrappers are intentionally disabled; the subdomain middleware still sets the
     * current tenant() which the global scope reads. Structure is kept clean so a single
     * upazila can be exported to its own DB later by re-enabling DatabaseTenancyBootstrapper.
     */
    'bootstrappers' => [
        // (empty) — single-database mode.
    ],
