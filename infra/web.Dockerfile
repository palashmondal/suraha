...
# vite.config.ts sets envDir to ../infra, so the app lives at /app/web and the VITE_* build
# vars are written to /app/infra/.env.production below.
WORKDIR /app/web
...
RUN mkdir -p /app/infra \
 && printf 'VITE_API_BASE=%s\nVITE_GOOGLE_MAPS_KEY=%s\n' \
      "$VITE_API_BASE" "$VITE_GOOGLE_MAPS_KEY" > /app/infra/.env.production \
 && npm run build
